\documentclass{phonologyproceedings}
...
\author{Presley Pizzo and Joe Pater}
\title{Does Learning Alternations Affect Phonotactic Judgments?}
...
opts_chunk$set(fig.width=4, fig.height=4, eval=TRUE, cache=FALSE, echo=FALSE, message=FALSE, warning=FALSE)
...
%     produces the different alternants of the morpheme.
    ...
    \citep{jakobson_mutual_1978}.  
    \citet{SPE} advocated against redundantly
    ...
    %deletion rules avoid them. 
    In order to capture the insight that multiple generalizations
    are driven by one force, %such as a constraint *CCC
    %or *\textsc{ComplexCoda}, 
    we must assume that phonotactics and alternations
...
%     Similarly, derived environment effects occur when a process
%     applies only in derived contexts, that is, contexts in which some other process
%     has applied or an affix has been added \citep{Kiparsky1973a,Mascaro1976,Wolf2008}. 
%     Thus, the phonotactic system tolerates
...
%     have a particular sequence, suggesting a phonotactic ban, but that sequence
...
%     \citep{hammond_phonology_1999}. 
    
    %Depending on theoretical assumptions, these
...
%     phonotactics and alternations are independent. 
%     At nine months of age, infants
...
%     they do not seem to be able to acquire alternations. 
%     \citet{white_rapid_2008}
...
%     alternations.
% 
%     That alternations are acquired later than phonotactics is to be
...
%     generalization that they have learned.
    ...
    true of some constraint-based models in the Optimality Theory tradition. 
%     \citet{Adriaans2010}
...
%     the learning of phonotactics when lexical information is not available.
    For instance, \citet{Hayes2008} model the induction of phonotactic constraints from words, but these words
...
\section{Experiment 1}
    ...
    of the phonological grammar, we conducted an experiment to test whether learning
    ...
    of the alternation. Thus, we use an artificial language learning paradigm.
    ...
    \subsection{Method}
        ...
        voiceless obstruent (specifically [f]), referred to as *DF, and one against a nasal followed by an obstruent of a different place (also [f]), referred to as *MF.
        One rule was constructed to repair each constraint: disagreement in voicing is
        repaired by devoicing the first obstruent (Devoicing), and place disagreement is repaired by changing
        the place of the nasal (Place Assimilation).
...
%         \end{math}
        ...
        English lexicon and the productive processes of English \citep{Kiparsky1985,Jun2004,darcy_phonological_2009}. 
%         While regressive
...
%         nasals to labial place \citep{Jun2004}. 
        %As a result, we can expect variability
        %in participants' learning of both processes --- that is, neither rule is
        %expected to be unlearnable or applied without exception. 
        %The rules are not
        %equally learnable for English speakers, but 
        However, the design of the experiment does
        not require them to be equally learnable, as the experiment is designed to test the interaction
        ...
        \subsubsection{Participants}
            ...
            In place of catch trials, we further assessed attention and success at the task
...
\end{quote}
            ...
            Speriment \footnote{https://github.com/presleyp/Speriment}. If the participant had answered less than 80\% of the graded
    ...
    \subsection{Results}
        ...
        match. The same means are given, along with standard deviations, in \ref{tab:alternations-percents}.

%         \begin{table}[htbp]
%         \begin{center}
%         \caption{Percent of times a constraint violation was chosen by condition in Experiment 1.}
\ex. Percent of times a constraint violation was chosen by condition in Experiment 1\\
            \begin{tabular}{llll}
...
%         their background knowledge as an English speaker.
        ...
        English place assimilation rule. 
%         The absolute values of the slopes of
...
%         behaved in a reasonable manner, 
        This offers some reassurance that the task worked as
        ...
        @
        \caption{Interaction between effect of rule training and constraint testing in Experiment 1.}
        ...
        @
        \caption{Distribution of by-participant violation preference for each condition in Experiment 1.}
        ...
        rate after being trained on Place Assimilation. 
...
%         somewhat abruptly rather than tapering to a fine point. 
%        Only six participants are represented in the portion of the Devoicing-*NF violin below 40\%. However, the shape of the Place Assimilation-*DF violin is striking, and may be due to the fact that this condition is the one in which neither training nor English language knowledge militate against violations.
        ...
        @
        \caption{Distribution of by-item violation preference for each item in Experiment 1.}
...
%         Of note are the distributions for those who were
...
%         for violations. 
        As in the grand means and the by-participant data, the
        ...
        on.  The complete formula is given in \Next. The table in \ref{tab:alternations-coefficients} shows the coefficients
...
%         \begin{table}[htbp]
%         \begin{center}
%         \caption{Coefficients of mixed effects model for Experiment 1.}
\ex. Coefficients of mixed effects model for Experiment 1\\
        \begin{tabular}{lll}
...
%         \begin{figure}[htbp]
%         \begin{center}
        <<alternations-interaction-1, echo=FALSE, cache=FALSE, eval=FALSE>>=
...
%         \begin{figure}[htbp]
%         \begin{center}
        <<alternations-interaction-234, cache=FALSE, eval=FALSE>>=
...
\section{Experiment 2}
    ...
    The primary weakness of Experiment 1 is that the presence of feedback creates
    a confound. If participants update their phonotactic grammars of the
    artificial language based not just on the presence of the test words
    but also on the presence of feedback for the test words, it would cause participants to prefer forms that satisfy
    the constraint that motivates their active rule. It is unclear if this
    effect is plausible, because token frequency, which is increased by
    feedback while type frequency is held constant, has not been found to
    be predictive of phonotactic judgments \citep{albright_gradient_2008}.
    However, feedback may simply amplify attention.
In order to address this confound, a second experiment was performed. Experiment 2
    removes feedback from the design, resulting in an
    experiment with an exposure phase, one iteration of training where no
    feedback is given, and a testing phase.

    \subsection{Method}
        Experiment 2 is very similar to Experiment 1; the parts of the method
        that were different are described below.

    \subsubsection{Training Phase}
        Recall that the training phase in Experiment 1 consisted of questions
        ...
        phase in Experiment 2 used the same stimuli and question format, but
        ...
        comparison with Experiment 1. Rather, this phase was used to determine
    ...
    \subsubsection{Participants}
        ...
        All exclusion criteria from Experiment 1 were used except the number of
        iterations, which was not applicable because multiple iterations were
        not possible in Experiment 2. These criteria resulted in two
        ...
        iteration was used.  In Experiment 1, 46 of 99 participants reached
...
%         \begin{table}[htbp]
%         \begin{center}
%        \caption{Percent of times a constraint violation was chosen by condition in Experiment 2.}
        \ex. Percent of times a constraint violation was chosen by condition in Experiment 2\\
            \begin{tabular}{llll}
            \hline Rule Trained On & Constraint Tested On & Mean & Standard Deviation \\
            \hline
...
%         \end{table}
        ...
        for Experiment 2. Once again, we see that the slopes of the lines have
        different signs, indicating that training on a rule decreased
        preferences for violating the constraint that motivates that rule. Compared to Experiment 1, participants trained on Place Assimilation behave less differently on the two constraints. 
        ...
        legend('topright', c('Devoicing', 'Place Assimilation'), lty=c(2,1), title = 'Active Rule')
        @
        \caption{Interaction between rule training and constraint testing in Experiment 2.}
        ...
        *NF. This trend is less clear in the Place Assimilation condition.
        ...
        @
        \caption{Distribution of by-participant violation preference for each condition in Experiment 2.}
        ...
        clearer in the Place Assimilation condition. As in Experiment 1, items
        ...
        @
        \caption{Distribution of by-item violation preference for each item in Experiment 2.}
        \label{fig:alternations2-item-violin}
        \end{center}
        \end{figure}

        The same model structure that was used to analyze the data from
        Experiment 1 was used to analyze this data, although some random effects were omitted to allow the model to converge. The resulting formula is given in \Next. The coefficients found by the model are shown
        in \ref{tab:alternations2-coefficients}.
...
%         \begin{table}[htbp]
%         \begin{center}
%         \caption{Coefficients of mixed effects model for Experiment 2.}
\ex. Coefficients of mixed effects model for Experiment 2\\
        \begin{tabular}{lll}
        \hline Factor & Estimate & $p$-value \\
        \hline
...
%         \end{table}
        ...
        training and testing are not significant. Unlike in Experiment 1, the
    ...
    \subsection{Discussion}
        ...
        The results of Experiment 2 are inconclusive. The data show a trend in the same direction
        as Experiment 1, but the predicted interaction between training and testing is not significant.
        This could mean that the finding of Experiment 1 was indeed due to feedback, but it could also mean
        that without feedback, participants learned the generalization less well, resulting in noisier results.
        When variance increases, all else held equal, power decreases, so it is plausible that Experiment 2 failed
...
\section{General Discussion}
    ...
    alternations is used in making phonotactic judgments. Experiment 1 found support
    for the hypothesis, but may have inadvertently drawn participants' attention to words
    that biased them towards giving such results. Experiment 2 eliminated feedback, and as
    ...
    to Experiment 1. Further experimentation is needed to determine whether Experiment 2 merely lacked
    power or failed to show an effect because it the effect had been due to a confound.

    Nevertheless, the trend found in Experiment 2 makes it worthwhile to consider
    ...
    same, monolithic system, as Optimality Theory asserts, 
    ...
    but would argue that information is shared between them, at least from alternations
    to phonotactics.
    %information from alternations to phonotactics. 
...
\cascadillabibliography{software,HG,phonology}
