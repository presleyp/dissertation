graphics.off()
rm(list=ls(all=TRUE))
fileNameRoot="cumulativity_logistic" # for constructing output filenames
#source("openGraphSaveGraph.R")
require(rjags)         # Kruschke, J. K. (2011). Doing Bayesian Data Analysis:
# A Tutorial with R and BUGS. Academic Press / Elsevier.
#------------------------------------------------------------------------------
# THE MODEL.
# requires data definitions for y, x, nData, nPredictors
# how to do interactions? random effects?
modelstring = "
model {
for( i in 1 : nData ) {
y[i] ~ dbern( mu[i] )
...
readData = function(filename, predictedName, predictorNames){
  df = read.csv(filename)
  dataList = list(
    x = as.matrix(df[,predictorNames]) ,
    y = as.vector(as.matrix(df[,predictedName])) ,
...
filename = '~/dissertation/cumulativity/cumulativity_march17_dataframe.csv'
predictedName = "SelectedTest"
predictorNames = c("OnsetViolation", 
                   "CodaViolation", 
                   "Filler", 
                   "Subject", 
                   "Item", 
...
dataList = readData(filename, predictedName, predictorNames)
#------------------------------------------------------------------------------
# INTIALIZE THE CHAINS.

glmInfo = glm( dataList$y ~ dataList$x[,predictorNames[1]] +
                            dataList$x[,predictorNames[2]] +
                            dataList$x[,predictorNames[3]] +
                            dataList$x[,predictorNames[4]] +
                            dataList$x[,predictorNames[5]] +
                            dataList$x[,predictorNames[6]] +
                            dataList$x[,predictorNames[3]] +
                            dataList$x[,predictorNames[4]]
                            , family=binomial(logit) ) # R func.
