...
### Exclusions
current = exclude_old(df, '4.0')
current = exclude_before(current, 2015, 5, 9)
...
non_minors = exclude_minors(workers, '659')
...
test = test[learned.Segment != '']
catch = learned[learned.PageType == 'Catch']

test_groups = test.groupby(['Permutation', 'SelectedText', 'Violates'])
print test_groups['Violates'].count()
